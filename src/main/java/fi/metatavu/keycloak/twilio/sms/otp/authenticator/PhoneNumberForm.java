...
public class PhoneNumberForm extends UsernamePasswordForm {
    ...
    protected boolean validateForm(AuthenticationFlowContext context, MultivaluedMap<String, String> formData) {
        String stringPhoneNumber = formData.getFirst(PHONE_NUMBER_FORM_FIELD).replace(" ", "");

        if (!validatePhoneNumber(stringPhoneNumber)) {
        ...
        UserModel foundUser = context.getSession().users().getUsersStream(context.getRealm(), false)
                .filter(user -> stringPhoneNumber.equals(user.getFirstAttribute(PHONE_NUMBER_ATTRIBUTE_NAME)))
                .findFirst()
                .orElse(null);
